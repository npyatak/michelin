...
<div class="city" data-id="<?=($model && $model->status == Post::STATUS_ACTIVE) ?  $model->id : '';?>" style="<?=$model ?  'display: block;' : '';?>">
    ...
    <div class="text_block <?=($model && $model->type == Post::TYPE_VIDEO) ? 'show-video' : '';?>">
        <div class="city_img">
            <div class="video" style="<?=($model && $model->status == Post::STATUS_ACTIVE) ?  'background-image: url('.$model->srcUrl.');' : '';?>">
                <div id="launch-video" class="play-video" data-id="<?=($model && $model->status == Post::STATUS_ACTIVE) ?  $model->yt_id : '';?>" style="<?=($model && $model->type == Post::TYPE_IMAGE) ? 'display: none;' : '';?>"></div>
            </div>
        </div>
